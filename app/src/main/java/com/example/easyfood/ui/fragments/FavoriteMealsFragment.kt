...
class FavoriteMealsFragment : Fragment() {
    ...
    private lateinit var detailsMVVM: DetailsMVVM
    private lateinit var favoriteMealsAdapter: FavoriteMealsRecyclerAdapter

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = FragmentFavoriteMealsBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setUpFavoriteMealRecyclerView()

        detailsMVVM.observeUserMeals().observe(viewLifecycleOwner, Observer { meals ->
            favoriteMealsAdapter.setFavoriteMealsList(meals)
            binding.tvFavEmpty.visibility = if (meals.isEmpty()) View.VISIBLE else View.GONE
        })

        onFavoriteMealItemClick()
        onFavoriteMealItemLongClick()
    }

    private fun setUpFavoriteMealRecyclerView() {
        favoriteMealsAdapter = FavoriteMealsRecyclerAdapter()
    ...
    private fun onFavoriteMealItemClick() {
        favoriteMealsAdapter.onFavoriteClickListener = { meal ->
            val intent = Intent(activity, MealDetailesActivity::class.java).apply {
                putExtra(HomeFragment.MEAL_ID, meal.mealId.toString())
                putExtra(HomeFragment.MEAL_STR, meal.mealName)
                putExtra(HomeFragment.MEAL_THUMB, meal.mealThumb)
            }
            startActivity(intent)
        }
    }

    private fun onFavoriteMealItemLongClick() {
        favoriteMealsAdapter.onFavoriteLongClickListener = { meal ->
            // Handle long click, e.g., show a dialog to confirm deletion
        }
    }
}
